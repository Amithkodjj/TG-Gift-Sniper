...
DEV_MODE = os.getenv("DEV_MODE", "false").lower() == "true"  # Test gift purchases
MAX_PROFILES = int(os.getenv("MAX_PROFILES", "3"))  # Maximum profiles per user
PURCHASE_COOLDOWN = float(os.getenv("PURCHASE_COOLDOWN", "0.3"))  # Purchases per second
...
def DEFAULT_CONFIG(user_id: int) -> dict:
    """Дефолтная configурация: глобальные поля + список профилей."""
    return {
        "BALANCE": 0,
        "ACTIVE": False,
...
async def ensure_config(user_id: int, path: str = CONFIG_PATH):
    """
    Ensures existence config.json.
    """
    if not os.path.exists(path):
        async with aiofiles.open(path, mode="w", encoding="utf-8") as f:
            await f.write(json.dumps(DEFAULT_CONFIG(user_id), indent=2))
        logger.info(f"Создана configурация: {path}")


async def load_config(path: str = CONFIG_PATH) -> dict:
    """
    Loads config from file (without validation). Гарантирует, что файл существует.
    """
    if not os.path.exists(path):
...
async def validate_config(config: dict, user_id: int) -> dict:
    """
    Validates global config и все профили.
    """
    valid = {}
...
async def get_valid_config(user_id: int, path: str = CONFIG_PATH) -> dict:
    """
    Loads, validates and updates if necessary config.json.
    """
...
async def migrate_config_if_needed(user_id: int, path: str = CONFIG_PATH):
    ...
    try:
        ...
        os.remove(path)
        logger.error(f"Повреждённый config {path} удалён.")
        return
...
async def get_profile(config: dict, index: int = 0) -> dict:
    ...
    profiles = config.get("PROFILES", [])
    if not profiles:
        raise ValueError("Нет профилей в configе")
    return profiles[index]


async def add_profile(config: dict, profile: dict, save: bool = True) -> dict:
    """
    Добавляет новый профиль в config.
    """
...
def format_config_summary(config: dict, user_id: int) -> str:
    """
    Forms text for main menu: status, balance, and list of all profiles (each with brief description).
    :param config: Вся configурация (словарь)
